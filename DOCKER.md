...
This document describes the Docker setup for the node-scaffold project.
...
4. **production** - Optimized production image with minimal footprint
...
- Ports: 3000 (backend), 27018 (MongoDB)
...
# MongoDB Connection
MONGODB_URL=mongodb://mongo:27017/node-scaffold
...
docker build --target production -t node-scaffold:prod .
docker build --target development -t node-scaffold:dev .

# Build with no cache
docker build --no-cache --target production -t node-scaffold:prod .
```
...
docker-compose ps
docker inspect node-scaffold-prod | grep -A 10 Health
```
...
docker stats node-scaffold-prod
```
...
- name: Build Docker image
  run: docker build --target production -t node-scaffold:${{ github.sha }} .
...
- name: Push to registry
  run: |
    docker tag node-scaffold:${{ github.sha }} registry.example.com/node-scaffold:latest
    docker push registry.example.com/node-scaffold:latest
```
